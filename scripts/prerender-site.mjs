  ...
  "gamification-in-education": {
    ...
    text: [
      ...
      ...GAMIFICATION_GAME_LEVELS.flatMap((level) => [
        ...
        ...(level.gradeBands ? Object.entries(level.gradeBands).map(([band, text]) => `Grades ${band}: ${text}`) : []),
        ...(level.cautionCard ? level.cautionCard.items.map((item) => `${item.risk}: ${item.text}`) : []),
        ...(level.evidenceSnapshot ? level.evidenceSnapshot.map((item) => `${item.stat}. ${item.caveat}`) : []),
        ...(level.lessonBlueprint ? level.lessonBlueprint.steps.map((step) => `${step.label}: ${step.prompt}`) : []),
      ]),
